// Enhanced Cart System với Authentication Check
class CartSystem {
    constructor() {
        ...
        container.innerHTML = this.cart.map((item) => {
            ...
            <div class="cart-item" data-id="${item.id}">
                <div class="cart-item-image">
                    <img src="${safeImg}" alt="${this.escapeHtml(item.name)}" loading="lazy" onerror="this.onerror=null;this.src='${this.defaultImage}'">
                </div>
            ...
            <div style="background: linear-gradient(135deg, var(--white), var(--light-color)); padding: 30px; border-radius: 20px; max-width: 500px; width: 90%; box-shadow: var(--shadow-lg); border: 1px solid var(--muted-border);">
                ...
                <div style="background: var(--light-color); padding: 15px; border-radius: 10px; margin-bottom: 20px;">
                    <div style="display: flex; justify-content: space-between; margin-bottom: 8px;">
                        ...
                        <strong style="color: var(--dark-color);">${this.escapeHtml(currentUser.name)}</strong>
                    </div>
                    <div style="display: flex; justify-content: space-between; margin-bottom: 8px;">
                        <span style="color: var(--text-color);">Email:</span>
                        <span style="color: var(--text-color);">${this.escapeHtml(currentUser.email)}</span>
                    </div>
                    <div style="display: flex; justify-content: space-between; font-size: 1.2rem; font-weight: bold;">
                        <span style="color: var(--dark-color);">Tổng tiền:</span>
                        <span style="color: var(--primary-color);">${this.formatCurrency(total)}</span>
                    </div>
                </div>
                    ...
                    <div style="max-height: 150px; overflow-y: auto;">
                        ${this.cart.map(item => `
                            <div style="display: flex; justify-content: space-between; padding: 5px 0; border-bottom: 1px solid var(--muted-border);">
                                <span style="color: var(--text-color);">${this.escapeHtml(item.name)} x${item.quantity}</span>
                                <span style="color: var(--text-color);">${this.formatCurrency(item.price * item.quantity)}</span>
                            </div>
                        `).join('')}
                    </div>
                </div>
    ...
    saveOrder(order) {
        try {
            ...
            orders.unshift(order); // Thêm vào đầu mảng
